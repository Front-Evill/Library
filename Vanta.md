...
`AddSectionsBox` creates a square card (image + title) instead of a normal
section. Cards from the same tab automatically arrange two per row. Clicking
a card fades the whole tab's normal content out and fades the card's
elements in, with a back arrow (top-right) to return. Only one card can be
open at a time per tab. The returned object supports every element method a
regular section does (`AddButton`, `AddToggle`, `AddSlider`, `AddDropdown`,
etc.) since it's the same underlying Section type.
